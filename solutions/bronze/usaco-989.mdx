...
```py
with open(r"race.in") as I:
    K, N = [int(x) for x in I.readline().split()]
    Xs = []
    for _ in range(N):
        Xs.append(int(I.readline()))


output = []
for X in Xs:
	# Greedy Algorithm was too slow.
	# Binary search ... but no upper bound
    l = 0
    r = X
    out = float("inf")
    while r != l:
        if r > X:
			# remaining_distance is the distance left between acceleration and deceleration
            remaining_distance = K - ((r + 1) * r / 2 + (r + X - 1) * (r - X) / 2)
            if remaining_distance < 0:
                r = (r - l) // 2 + l
                continue
            time = -(-remaining_distance // r) + r + (r - X)  # Funny way to do ceiling
        else:
            remaining_distance = K - (r + 1) * r / 2
            if remaining_distance < 0:
                r = (r - l) // 2 + l
                continue
            time = -(-remaining_distance // r) + r
        out = min(out, time)
        l = r
        r = 2*r
    output.append(int(out))

with open(r"race.out",mode="w") as O:
    for out in output:
        O.write(str(out)+"\n")
